...
class StatusReporter:
    def __init__(self, bot_id, data_dir="/bot/data", status_web_url=None, github_token=None, status_repo=None):
        self.bot_id = bot_id
        self.data_dir = Path(data_dir)
        self.status_web_url = status_web_url or "http://claude-status-web:5000"
        self.github_token = github_token
        self.status_repo = status_repo  # Format: "owner/repo"
        self.start_time = datetime.now()
    ...
    def publish_to_github(self, status_data):
        ...
        if not self.github_token or not self.status_repo:
            print("⚠️  GitHub token or repository not configured, skipping GitHub publish")
            return False
        
        try:
            # GitHub API URL for file updates
            api_url = f"https://api.github.com/repos/{self.status_repo}/contents/status/{self.bot_id}/current.json"
            ...
            import base64
            content_bytes = json.dumps(status_data, indent=2).encode('utf-8')
            content_base64 = base64.b64encode(content_bytes).decode('utf-8')
            
            payload = {
                "message": f"Update {self.bot_id} status - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            ...
            if response.status_code in [200, 201]:
                print(f"✅ Status published to GitHub: {self.status_repo}")
                return True
            else:
                print(f"❌ Failed to publish to GitHub: {response.status_code} - {response.text}")
                return False
    ...
    def generate_and_publish(self):
        ...
        # Also publish to GitHub if configured (backup/external access)
        if self.github_token and self.status_repo:
            self.publish_to_github(status_data)
...
def main():
    ...
    parser.add_argument('--data', default='/bot/data', help='Bot data directory')
    parser.add_argument('--web-url', help='Status web dashboard URL (default: http://claude-status-web:5000)')
    parser.add_argument('--github-token', help='GitHub token for publishing (optional)')
    parser.add_argument('--status-repo', help='GitHub repository for status (owner/repo) (optional)')
    parser.add_argument('--loop', action='store_true', help='Run continuously every 5 minutes')
    parser.add_argument('--interval', type=int, default=300, help='Loop interval in seconds (default: 300)')
    ...
    status_repo = args.status_repo or os.getenv('STATUS_REPO')
    web_url = args.web_url or os.getenv('STATUS_WEB_URL')
